  ...
  return (
    <div style={styles.app}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div style={{ ...styles.card, width: "100%", maxWidth: 420, padding: 16, textAlign: "center" }}>
          <div style={{ fontSize: 11, color: C.inkSoft, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
            Croupier {phase !== "betting" && !dealerHidden ? `(${dVal.total}${dVal.soft ? " souple" : ""})` : ""}
          </div>
          <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap", minHeight: 70 }}>
            {dealerCards.map((c, i) => <PlayingCard key={i} card={c} hidden={dealerHidden && i === 1} C={C} />)}
          </div>

          <div style={{ borderTop: `1px solid ${C.line}`, margin: "14px 0" }} />

          <div style={{ fontSize: 11, color: C.inkSoft, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
            Vous {playerCards.length > 0 ? `(${pVal.total}${pVal.soft ? " souple" : ""})` : ""}
          </div>
          <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap", minHeight: 70 }}>
            {playerCards.map((c, i) => <PlayingCard key={i} card={c} C={C} />)}
          </div>

          {resultMsg && (
            <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 10, border: `1.5px dashed ${resultMsg.payout > 0 ? C.good : resultMsg.payout < 0 ? C.bad : C.inkSoft}` }}>
              <div style={{ fontWeight: 700, color: C.ink, fontSize: 13 }}>{resultMsg.outcome}</div>
              {resultMsg.payout !== 0 && <div style={{ fontSize: 12, color: resultMsg.payout > 0 ? C.good : C.bad, marginTop: 2 }}>{resultMsg.payout > 0 ? "+" : ""}{f(resultMsg.payout)}</div>}
            </div>
          )}
        ...
        <div style={{ fontSize: 10, color: C.inkSoft, marginTop: 14, textAlign: "center", maxWidth: 340 }}>
