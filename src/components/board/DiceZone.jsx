...
export default function DiceZone({ dice, diceRolling, onRoll, skipTurns, extra, pending, moving }) {
  const busy = diceRolling || pending || moving;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginTop: 18, gap: 10 }}>
      ...
      <button className="btn-primary" style={{ ...styles.primaryBtn, opacity: busy ? 0.5 : 1 }} disabled={busy} onClick={onRoll}>
        {pending ? "Décision en attente…" : moving ? "En chemin…" : skipTurns > 0 ? `Passer le tour (${skipTurns} restant${skipTurns > 1 ? "s" : ""})` : diceRolling ? "…" : "Lancer le dé"}
      </button>
    </div>
  );
}
