...
export default function SetupScreen({ onStart, currency, onSelectCurrency, onBack, customJobs, onCreateJob, onDeleteJob, mpSetupLabel }) {
  ...
  return (
    <div className="screen-in" style={{ ...styles.app, overflowX: "hidden", overflowY: "auto", padding: "26px 18px 40px", width: "100%", minWidth: 0, boxSizing: "border-box" }}>
      ...
      {mpSetupLabel && (
        <div style={{ textAlign: "center", fontSize: 12, color: COLORS.teal, fontWeight: 700, marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>{mpSetupLabel}</div>
      )}
      <div style={{ textAlign: "center", marginBottom: 20, width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ fontFamily: DISPLAY_FONT, fontSize: 34, lineHeight: 1.05, color: COLORS.ink, fontWeight: 700, letterSpacing: 0.2, overflowWrap: "anywhere" }}>Liberté Financière</div>
      ...
      <div style={styles.profGrid}>
        {PROFESSIONS.map((p) => {
          ...
          return (
            <button key={p.id} className="prof-card" style={styles.profCard} onClick={() => setConfiguring(p)}>
              ...
              <div style={{ fontFamily: DISPLAY_FONT, fontSize: 14, color: COLORS.ink, marginTop: 4, fontWeight: 700 }}>{p.name}</div>
        ...
        <button className="prof-card" style={{ ...styles.profCard, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: COLORS.inkSoft }} onClick={onCreateJob}>
          ...
          <div style={{ fontFamily: DISPLAY_FONT, fontSize: 13, marginTop: 4, fontWeight: 700 }}>Créer un métier</div>
        </button>
      </div>
    </div>
  );
